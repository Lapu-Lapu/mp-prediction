...
with pm.Model() as multilevel:
    ...
    a_bar = pm.Normal('a_bar', a_barbar, 1.5, shape=len(mp_types))
    sigma_a = pm.Exponential('sigma_a', 1, shape=len(mp_types))
    b_barbar = pm.Normal('b_barbar', 0, 1.5)
    b_bar = pm.Normal('b_bar', b_barbar, 1.5, shape=len(mp_types))
    ...
    p = c[df.id_mptype] / (1 + np.exp(-(a_bar[df.id_mptype] + sigma_a[df.id_mptype] * a[df.id_participant] + (b_bar[df.id_mptype] + sigma_b[df.id_mptype] * b[df.id_participant]) * x)))
