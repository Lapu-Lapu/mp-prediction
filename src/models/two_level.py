...
with pm.Model() as multilevel:
    ...
    # p = c[df.id_mp_type] / (1 + np.exp(-((a_barbar + sigma_abar * a_bar[df.id_mp_type]) + sigma_a[df.id_mp_type] * a[df.id_participant] + (b_bar[df.id_mp_type] + sigma_b[df.id_mp_type] * b[df.id_participant]) * x)))
