...
with pm.Model() as multilevel:
    a_bar = pm.Normal('a_bar', 0, 1.5, shape=2)
    sigma_a = pm.Exponential('sigma_a', 1, shape=2)
    a = pm.Normal('a', 0, 1, shape=(len(participants), 2))
    p = 1 / (1 + np.exp(
        -(a_bar[df.occ] + (sigma_a[df.occ]
...
with open("data/processed/segmentation_effect_trace_pooled.pkl", "wb") as fh:
